...
def main():
	...
	start = time.clock()
	fos_cl_fista_results = test_X_FOS( X, y, hdim.SolverType_cl_fista )
	end = time.clock()

	print( "CL FISTA timing ", end - start )
	...
	#rects6 = ax.bar(ind + 5*width, fos_cl_fista_results, width, color='r')

	#ax.legend((rects1[0], rects2[0], rects3[0], rects4[0], rects5[0], rects6[0]), ('ISTA', 'FISTA', 'SCREEN_FISTA', 'CD', 'SCREEN_CD', 'CL_FISTA'))
